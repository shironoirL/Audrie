...
from .models import (Drug, Disease, DrugDiseaseProbability, PathPrediction, MetaPathPrediction, SourceEdgePrediction,
                     TargetEdgePrediction, DiseaseIndication, MechanismOfAction)
...
class PathPredictionForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            'drug_disease_probability': autocomplete.ModelSelect2(url='path-predictions-autocomplete', forward=['length']),
        }

class PathPredictionAdmin(admin.ModelAdmin):
    form = PathPredictionForm
    list_display = ['verbose_path', 'drug_disease_probability', 'percent_of_prediction', 'percent_of_dwpc', 'metapath', 'length']
    search_fields = ['drug_disease_probability__drug__name', 'drug_disease_probability__disease__name']
...
class MetaPathPredictionAdmin(admin.ModelAdmin):
    list_display = ['metapath', 'drug_disease_probability', 'percent_of_prediction', 'path_count', 'length', 'verbose']
    search_fields = ['drug_disease_probability__drug__name', 'drug_disease_probability__disease__name']
...
class SourceEdgePredictionAdmin(admin.ModelAdmin):
    list_display = ['source_edge', 'drug_disease_probability', 'percent_of_prediction', 'path_count', 'distinct_metapaths']
    search_fields = ['drug_disease_probability__drug__name', 'drug_disease_probability__disease__name']
...
class TargetEdgePredictionAdmin(admin.ModelAdmin):
    list_display = ['target_edge', 'drug_disease_probability', 'percent_of_prediction', 'path_count', 'distinct_metapaths']
    search_fields = ['drug_disease_probability__drug__name', 'drug_disease_probability__disease__name']
...
class DiseaseIndicationForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['chembl_id', 'disease', 'efo_name', 'reference_link', 'max_phase_for_indication']
class DiseaseIndicationAdmin(admin.ModelAdmin):
    list_display = ['chembl_id', 'disease', 'efo_name', 'reference_link', 'max_phase_for_indication']
...
class MechanismOfActionForm(forms.ModelForm):
    class Meta:
        ...
        fields = ['chembl_id', 'action_type', 'mechanism_of_action', 'target_name', 'target_type', 'target_list', 'reference_list']

class MechanismOfActionAdmin(admin.ModelAdmin):
    list_display = ['chembl_id', 'action_type', 'mechanism_of_action', 'target_name', 'target_type', 'target_list', 'reference_list']
